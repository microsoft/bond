...
/**
 * Implements the {@link BondType} contract for the Bond "uint64" data type.
 */
public final class UInt64BondType extends PrimitiveBondType<Long> {
    ...
    protected static long deserializePrimitiveField(
            TaggedDeserializationContext context,
            StructBondType.StructField<Long> field) throws IOException {
        // an uint64 value may be deserialized from BT_UINT64, BT_UINT32, BT_UINT16 or BT_UINT8
        if (context.readFieldResult.type.value != BondDataType.BT_UINT64.value) {
            if (context.readFieldResult.type.value == BondDataType.BT_UINT32.value) {
                return context.reader.readUInt32();
            } else if (context.readFieldResult.type.value == BondDataType.BT_UINT16.value) {
