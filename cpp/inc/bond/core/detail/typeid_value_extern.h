...
#define BOND_DETAIL_INSTANTIATE_MapByKey(Reader, Writer, T) \
    template void MapByKey<BuiltInProtocols, const Serializer<Writer>, value<T, Reader&> >(const Serializer<Writer>&, BondDataType, const value<T, Reader&>&, Reader&, uint32_t);
#else
#define BOND_DETAIL_INSTANTIATE_MapByKey(Reader, Writer, T) \
    template void MapByKey<BuiltInProtocols>(const Serializer<Writer>&, BondDataType, const value<T, Reader&>&, Reader&, uint32_t);
#endif
...
#define BOND_DETAIL_INSTANTIATE_MapByElement(Reader, Writer) \
    template void MapByElement<BuiltInProtocols>(const Serializer<Writer>&, BondDataType, BondDataType, Reader&, uint32_t);
