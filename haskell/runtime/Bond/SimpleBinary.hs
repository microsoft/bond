...
instance BondBinary SimpleBinaryV1Proto FieldTag where
    bondPut _ = return ()
    bondGet = fail "internal error: can't read field tag from SimpleBinaryV1"
...
getBondedContainer :: BondGet SimpleBinaryV1Proto (Bonded a)
...
putBondedContainer :: Bonded a -> BondPut SimpleBinaryV1Proto
putBondedContainer (BondedStream s _) = do
    BondPut $ putWord32le $ fromIntegral (Lazy.length s)
    BondPut $ putLazyByteString s

readSimpleBinaryStruct :: forall a. BondBinaryStruct SimpleBinaryV1Proto a => (a -> ItemType -> Ordinal -> BondGet SimpleBinaryV1Proto a) -> a -> BondGet SimpleBinaryV1Proto a
readSimpleBinaryStruct update r = foldM (\v (FieldInfo _ o) -> update v undefined o) r schema
    where
    StructSchema schema = bondGetSchema r :: StructSchema SimpleBinaryV1Proto
